...
const AnalysisScreen: React.FC<AnalysisScreenProps> = ({ session, transcripts, onRestart }) => {
  // FIX: Treat scores as an array of DetailedScore objects instead of primitives
  const validScores = session.state.scores;
  const avgAggregate = validScores.length > 0 
    ? (validScores.reduce((a, b) => a + b.aggregateScore, 0) / validScores.length) 
    : 0;
    ...
    <div className="w-full max-w-6xl bg-white rounded-[3.5rem] shadow-3xl overflow-hidden border border-slate-100 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="bg-slate-900 p-12 lg:p-16 text-white flex flex-col lg:flex-row justify-between items-end gap-10 border-b border-white/5">
        <div className="space-y-4">
          ...
          <h2 className="text-5xl font-black tracking-tight uppercase">Assessment Report</h2>
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px]">
            Session: {session.sessionId} • Domain Mastery: {session.domain}
          </p>
        </div>
        <div className="text-center bg-white/5 px-10 py-8 rounded-[2.5rem] border border-white/5 backdrop-blur-md shrink-0">
          <p className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.3em] mb-3 italic">Aggregate Mastery</p>
      ...
      <div className="p-10 lg:p-16 grid grid-cols-1 xl:grid-cols-3 gap-16">
        <div className="xl:col-span-2 space-y-12">
          <section>
             <div className="flex items-center justify-between mb-8">
               <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Performance Timeline</h3>
               <span className="text-[9px] font-bold text-slate-300 uppercase">Assessment Cycles: {validScores.length}</span>
             </div>
             <div className="space-y-6 max-h-[600px] overflow-y-auto pr-6 custom-scrollbar">
               {transcripts.filter(t => t.speaker === 'user').map((t, i) => (
                 <div key={i} className="p-8 bg-slate-50/50 rounded-[2rem] border border-slate-100 hover:border-indigo-100 transition-all hover:bg-white hover:shadow-xl group">
                    <div className="flex justify-between items-center mb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Logic Packet</span>
                      </div>
                      <div className="flex items-center gap-3">
                         {/* FIX: Access aggregateScore property for display */}
                         <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest">Score: {Math.round((session.state.scores[i]?.aggregateScore || 0) * 100)}%</span>
                         <div className="w-16 h-1 bg-slate-200 rounded-full overflow-hidden">
                            {/* FIX: Access aggregateScore property for width calculation */}
                            <div className="h-full bg-indigo-500" style={{ width: `${(session.state.scores[i]?.aggregateScore || 0) * 100}%` }} />
                         </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-5 border-t border-slate-100">
                      <p className="text-xs text-slate-500 font-bold leading-relaxed">
                        ...
                        {session.state.qualitativeFeedback[i+1] || "Consistent technical delivery observed."}
                      </p>
                    </div>
                 </div>
               ))}
        ...
        <aside className="space-y-10">
          <div className="space-y-6">
            <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Engine Configuration</h3>
          ...
          <div className="p-10 bg-indigo-600 rounded-[2.5rem] text-white shadow-3xl shadow-indigo-200">
            ...
            <p className="text-2xl font-black leading-tight mb-8">
              {avgAggregate > 0.8 ? 'Superior technical alignment detected across all assessment topics.' : 
               avgAggregate > 0.6 ? 'Adequate domain proficiency confirmed with slight variance in delivery.' : 
               'Proficiency foundational; further technical depth recommended in target domain.'}
            </p>
            <button 
              onClick={onRestart}
